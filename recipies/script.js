...
function generateTagsHtml(tags) {
    ...
    return tags.map(tag => `<span class="tag">${tag.toLowerCase()}</span>`).join(' ');
}
...
function renderRecipes(recipesToRender) {
    ...
    if (recipesToRender.length === 0) {
        mainContainer.innerHTML = '<p style="text-align: center; font-size: 1.2rem; padding: 20px;">No recipes found matching your search.</p>';
        return;
    }
    ...
    recipesToRender.forEach(recipe => {
        ...
        article.innerHTML = `
            ...
            <div class="recipe-content">
                <div>${generateTagsHtml(recipe.tags)}</div>
...
function handleSearch() {
    const searchInput = document.querySelector('.search-area input').value.toLowerCase().trim();
    ...
    const filteredRecipes = recipes.filter(recipe => {
        ...
        const matchDescription = recipe.description.toLowerCase().includes(searchInput);
        
        const matchTags = recipe.tags && recipe.tags.some(tag => tag.toLowerCase().includes(searchInput));
